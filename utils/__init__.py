...
from sanjab.utils.highlighting import Highlighter
...
def _lookup_identifier_method():
    """
    If the user has set SANJAB_IDENTIFIER_METHOD, import it and return the method uncalled.
    If SANJAB_IDENTIFIER_METHOD is not defined, return sanjab.utils.default_get_identifier.
    ...
    of this module.
    """
    if not hasattr(settings, 'SANJAB_IDENTIFIER_METHOD'):
        return default_get_identifier

    module_path, method_name = settings.SANJAB_IDENTIFIER_METHOD.rsplit(".", 1)

    try:
        module = importlib.import_module(module_path)
    except ImportError:
        raise ImportError(u"Unable to import module '%s' provided for SANJAB_IDENTIFIER_METHOD." % module_path)

    identifier_method = getattr(module, method_name, None)

    if not identifier_method:
        raise AttributeError(
            u"Provided method '%s' for SANJAB_IDENTIFIER_METHOD does not exist in '%s'." % (method_name, module_path)
        )
