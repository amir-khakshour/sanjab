...
try:
    ...
    from sanjab.exceptions import NotHandled as IndexNotFoundException
    legacy = False
except ImportError:
    try:
        from sanjab import site
        from sanjab.exceptions import NotRegistered as IndexNotFoundException  # noqa
        legacy = True
    except ImportError as e:
        raise ImproperlyConfigured("Sanjab couldn't be imported: %s" % e)

if SANJAB_CELERY_TRANSACTION_SAFE and not getattr(settings, 'CELERY_ALWAYS_EAGER', False):
...
class CelerySanjabSignalHandler(Task):
    using = SANJAB_CELERY_DEFAULT_ALIAS
    max_retries = SANJAB_CELERY_MAX_RETRIES
    default_retry_delay = SANJAB_CELERY_RETRY_DELAY
...
class CelerySanjabUpdateIndex(Task):
    """
    ...
    def run(self, apps=None, **kwargs):
        defaults = {
            'batchsize': settings.SANJAB_CELERY_COMMAND_BATCH_SIZE,
            'age': settings.SANJAB_CELERY_COMMAND_AGE,
            'remove': settings.SANJAB_CELERY_COMMAND_REMOVE,
            'using': [settings.SANJAB_CELERY_DEFAULT_ALIAS],
            'workers': settings.SANJAB_CELERY_COMMAND_WORKERS,
            'verbosity': settings.SANJAB_CELERY_COMMAND_VERBOSITY,
        }
        defaults.update(kwargs)
        if apps is None:
            apps = settings.SANJAB_CELERY_COMMAND_APPS
