...
from sanjab.utils import get_model_ct_tuple
...
class SearchChangeList(ChangeList):
    def __init__(self, **kwargs):
        self.sanjab_connection = kwargs.pop('sanjab_connection', 'default')
    ...
    def get_results(self, request):
        ...
        sqs = SearchQuerySet(self.sanjab_connection).models(self.model).auto_query(request.GET[SEARCH_VAR]).load_all()

        paginator = Paginator(sqs, self.list_per_page)
        # Get the number of objects, with admin filters applied.
        result_count = paginator.count
        full_result_count = SearchQuerySet(self.sanjab_connection).models(self.model).all().count()
...
class SearchModelAdmin(ModelAdmin):
    # sanjab connection to use for searching
    sanjab_connection = 'default'
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # returned bits.
        if not self.model in connections[self.sanjab_connection].get_unified_index().get_indexed_models():
        ...
        kwargs = {
            'sanjab_connection': self.sanjab_connection,
            'request': request,
            'model': self.model,
