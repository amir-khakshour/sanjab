...
from sanjab.constants import VALID_FILTERS, FILTER_SEPARATOR, DEFAULT_ALIAS
from sanjab.exceptions import MoreLikeThisError, FacetingError
from sanjab.models import SearchResult
from sanjab.utils.loading import UnifiedIndex
from sanjab.utils import get_model_ct
...
class BaseSearchQuery(object):
    ...
    implementation.
    """
    def __init__(self, doc_types=[], using=DEFAULT_ALIAS):
    ...
    def build_params(self, spelling_query=None):
        ...
        if self.doc_types:
            kwargs['doc_types'] = self.doc_types
    ...
    def run(self, spelling_query=None, **kwargs):
        ...
        self._aggregations = results.get('aggregations', {})
    ...
    def run_raw(self, **kwargs):
        ...
        if kwargs:
            search_kwargs.update(kwargs)
        results = self.backend.raw_search(self._raw_query, **search_kwargs)
        self._results = results.get('results', [])
        self._hit_count = results.get('hits', 0)
    ...
    def add_filter(self, query_filter, use_or=False):
        ...
        @see SearchQuerySet.filter_and()
        @see SearchQuerySet.filter_or()
        """
        if use_or:
            connector = SQ.OR
        else:
    ...
    def add_within(self, field, point_1, point_2):
        ...
        from sanjab.utils.geo import ensure_point
        self.within = {
            'field': field,
    ...
    def add_dwithin(self, field, point, distance):
        ...
        from sanjab.utils.geo import ensure_point, ensure_distance
        self.dwithin = {
            'field': field,
    ...
    def set_result_class(self, klass):
        ...
        Overrides any previous usages. If ``None`` is provided, Sanjab will
    ...
    def _clone(self, klass=None, using=None):
        ...
        clone.doc_types = self.doc_types
...
class BaseEngine(object):
    ...
    def __init__(self, using=None):
        ...
        self.using = using
        self.options = settings.SANJAB_CONNECTIONS.get(self.using, {})
        self.queries = []
        self._index = None
        self._backend = None
