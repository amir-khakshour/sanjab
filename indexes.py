...
from sanjab import connection_router, connections
from sanjab.constants import DEFAULT_ALIAS, DJANGO_CT, DJANGO_ID, ID, Indexable
from sanjab.fields import *
from sanjab.manager import SearchIndexManager
from sanjab.utils import get_facet_field_name, get_identifier, get_model_ct
...
class DeclarativeMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        # Build a dictionary of base fields
        built_fields = {}
        ...
        attrs['fields'] = collections.OrderedDict(
            sorted(attrs['fields'].iteritems(), key=lambda x: x[1].weight))
...
class SearchIndex(with_metaclass(DeclarativeMetaclass, threading.local)):
    ...
    """
    base = False  # Base Index for model
    type = None  # Base Index for model
    ...
    def build_queryset(self, using=None, start_date=None, end_date=None):
        ...
        if hasattr(self, 'get_queryset'):
            warnings.warn("'SearchIndex.get_queryset' was deprecated in Sanjab v2. Please rename the method 'index_queryset'.")
    ...
    def update_object(self, instance, doc_type, using=None, **kwargs):
        """
        ...
        if self.should_update(instance):
            ...
            if backend is not None:
                backend.update(self, doc_type, [instance])
    ...
    def remove_object(self, instance, doc_type, using=None, **kwargs):
        """
        ...
        if backend is not None:
            backend.remove(instance, doc_type, **kwargs)
...
class RealtimeSeachIndex(SearchIndex):
    pass
...
def index_field_from_django_field(f, default=CharField):
    """
    Returns the Sanjab field type that would likely be associated with each
    Django type.
    """
    result = default
