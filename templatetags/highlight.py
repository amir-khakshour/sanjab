...
class HighlightNode(template.Node):
    ...
    def render(self, context):
        ...
        if hasattr(settings, 'SANJAB_CUSTOM_HIGHLIGHTER') and settings.SANJAB_CUSTOM_HIGHLIGHTER:
            # Do the import dance.
            try:
                path_bits = settings.SANJAB_CUSTOM_HIGHLIGHTER.split('.')
                ...
                raise ImproperlyConfigured("The highlighter '%s' could not be imported: %s" % (settings.SANJAB_CUSTOM_HIGHLIGHTER, e))
        else:
            from sanjab.utils import Highlighter
